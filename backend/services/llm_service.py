...
**执行计划诊断**：
  ...
  "operation": "<主要操作类型，如 TABLE ACCESS FULL>",
  "table": "<主表名>",
  "cost_indicator": "<高/中/低>",
  "bottleneck": "<简述瓶颈，如：全表扫描导致行数过多 / Using filesort>",
  "rows_scanned": <估算扫描行数>


**元数据分析**：
- 表结构特征：<简述关键字段类型、现有索引情况>
- 潜在风险：<如：字段类型不匹配风险 / 缺少分区 / 索引冗余>

**优化方案**：

/* 表结构改造 */
<若无需修改表结构，输出 "无需表结构改造">
<若需修改，提供准确的 ALTER TABLE 语句>
- 索引策略说明：<解释为何创建该索引，确认无重复索引，说明列顺序理由（如 is_delete 置后）>
示例：
ALTER TABLE <table_name> ADD INDEX idx_<name> (<col1>, <col2>, <is_delete_col>);

/* SQL重写 */
<若无需重写，输出 "无需重写" 并列出命中的优化规则编号，如 "命中规则：rule01, rule08">
<若需重写，提供完整的、语义等价的优化后 SQL>
<在关键改动处添加简短注释，如：-- 消除隐式转换 / -- 投影下推>
<优化后的 SQL 语句>

**预期收益**：
▸ 执行耗时：<预估优化前> → <预估优化后>
▸ 逻辑读/扫描行数：<优化前量级> → <优化后量级>
▸ 核心提升点：<一句话总结，如：通过覆盖索引消除回表，避免文件排序>
```
...
5. **区分表名**: 分析时要注意区分表的别名，推荐索引创建时别创建错。
"""
        return full_prompt
