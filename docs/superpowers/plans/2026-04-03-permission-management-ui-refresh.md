# Permission Management UI Refresh Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将权限管理页改造成简洁双栏授权台，在不改后端接口和 store 契约的前提下补齐状态反馈、批量操作和更清晰的卡片式交互。

**Architecture:** 保留现有 `frontend/src/views/PermissionManagement.vue` 单文件组件入口，把用户筛选、当前用户摘要、连接筛选、全选/清空和保存禁用态全部收口到页面级 `computed` 与 helper 方法中。新增一个页面级 Vitest 文件，使用真实 Pinia store + API mock 覆盖管理员提示、普通用户授权、搜索空态与保存流程，确保改版只触及 UI 层而不回归现有业务链路。
...
- Modify: `frontend/src/views/PermissionManagement.vue`
  责任：将表格 + 复选框页面重构为双栏授权工作台，增加页面级计算属性、交互方法和新的轻量样式。
- Create: `frontend/src/views/PermissionManagement.spec.ts`
  责任：覆盖页面核心交互，包括普通用户选择、管理员点击提示、连接筛选、全选/清空、搜索空态与保存授权。

## Task 1: Lock The New Interaction Model With A View Test

**Files:**
- Create: `frontend/src/views/PermissionManagement.spec.ts`
- Modify: `frontend/src/views/PermissionManagement.vue`
- Test: `frontend/src/views/PermissionManagement.spec.ts`

- [ ] **Step 1: Write the failing page-level test**

`frontend/src/views/PermissionManagement.spec.ts`
...
```bash
cd frontend && npm test -- src/views/PermissionManagement.spec.ts
```

Expected: FAIL，现有页面没有 `.user-card`、`.toolbar-button`、`.permission-save` 这些新结构，且缺少已授权数量、搜索空态和全选/清空交互。

- [ ] **Step 3: Replace the page template and logic with the two-panel workbench**

`frontend/src/views/PermissionManagement.vue`

将现有 `<template>` 和 `<script setup>` 替换为下面的内容，先保留旧的 `<style scoped>`，样式在 Task 2 统一重写：
...
- [ ] **Step 4: Run the view test to verify the new interaction model passes**
...
Expected: PASS，5 个用例通过，说明用户选择、管理员提示、筛选空态、全选/清空和保存授权都已经连通。

- [ ] **Step 5: Commit the tested interaction refactor**

Run:

```bash
git add frontend/src/views/PermissionManagement.vue frontend/src/views/PermissionManagement.spec.ts
git commit -m "feat: refresh permission management interactions"
```

## Task 2: Apply The Final Visual Polish And Responsive Layout

**Files:**
- Modify: `frontend/src/views/PermissionManagement.vue`
- Test: `frontend/src/views/PermissionManagement.spec.ts`

- [ ] **Step 1: Replace the old scoped styles with the final light enterprise theme**
...
- [ ] **Step 2: Run targeted tests and the production build**
...
Expected:
- `npm test -- src/views/PermissionManagement.spec.ts` PASS
- `npm run build` PASS
- 如果仍出现 chunk size 或 dynamic import 提示，只记录为现有项目告警，不作为本次 UI 改造阻塞项
...
```bash
git add frontend/src/views/PermissionManagement.vue frontend/src/views/PermissionManagement.spec.ts
git commit -m "style: polish permission management workspace"
```
